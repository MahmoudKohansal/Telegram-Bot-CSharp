...
namespace MVC_EF_BOT.Classes
{
    public class TeleBot
    {
        ...
        static MVC_EF_BOT.Models.BotJoke botJoke;
        static List<MVC_EF_BOT.Models.BotJoke> jokeList;
        public TeleBot()
            {
        ...
        private static void Bot_OnMessage(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            ...
            msgFirst20Character = msgGist.Substring(0, 20);
            ...
            #region Joke
            if (msgFirst20Character.Contains("لطیفه"))
            {
                botJoke.joke = msg;
                botJoke.teleID = e.Message.Chat.Id;
                botCntrlr.StoreJoke(botJoke);
                return;
            }
            if (msgFirst20Character.Contains("جک") || msgFirst20Character.Contains("جوک"))
            {
                jokeList = botCntrlr.SelectAllJokes();
                int rndSeed = rndNumber.Next(0, jokeList.Count - 1);
                bot.SendTextMessageAsync(e.Message.Chat.Id, jokeList[rndSeed].joke);
                return;
            }
            #endregion
            //else
